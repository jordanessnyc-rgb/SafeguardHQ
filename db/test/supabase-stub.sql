-- Minimal stand-in for the pieces of Supabase that our migrations depend on, so migrations and RLS
-- tests can run against plain Postgres (local dev/CI). NOT applied to real Supabase projects.
do $$ begin
...
-- Same semantics as Supabase's auth.uid(): reads the `sub` claim of the request JWT.
